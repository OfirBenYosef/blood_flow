...
def FFT_module(vid,fs):
    ...
    gray_frame = np.array(mean_box(vid[:,:,:,1]))
    ...
    plt.stem(frequencies,filtered_magnitude)
    plt.xlim([-5,5])
    plt.show()
...
def save_video(video_tensor,fs):
    fourcc = cv2.VideoWriter_fourcc('M','J','P','G')
    [height,width]=video_tensor[0].shape[0:2]
    writer = cv2.VideoWriter("human_good.avi", fourcc, fs, (width, height), 1)
...
if __name__ == '__main__':
    ...
    vid, full_color, fs = load_video('good_hr.mov')
    frame = full_color[-1,:,:,:]
    hist =  color_hist(frame)

    vid, full_color, fs = load_video('bad_hr.mov')
    frame = full_color[-1,:,:,:]
    hist =  color_hist(frame)
    plot_chi(hist)
    print('.')
